...
- **中文优先**：gse 分词器提供准确的中文分词，内置 GSE IDF 词典辅助关键词提取
- **Markdown 感知**：分块时尊重标题和代码块边界（300 字符目标）
- **Agent 就绪**：MCP Server + JSON 输出，便于 AI Agent 集成
- **Agent 记忆**：统一存储在 documents+chunks 中，通过 `/query` 端点配合 `@episodic` / `@knowledge` 系统集合查询
...
| `search <query>` | BM25 关键词搜索（默认 DF 稀有词提取策略） |
...
| `status` | 查看索引状态（含 ETA 预测和每个集合的 chunk 数量） |
| `rebuild` | 清空数据并重建索引 |
| `stop` | 停止 daemon |
