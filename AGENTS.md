...
2. If lacking: add the rule under "memory.md", written as concretely as possible ("Always use X for Y" not "be careful with Y").
...
该文件由agent维护，单条简短清晰（小于200字），只包含3种类型的数据：

1. 关键决策，比如：RAG检索使用 hybrid search = bm25 + vector search
2. 文件索引 (地图)：指示agent在何时去何地查找某个文件
3. 经验教训：When the user corrects your approach, append a one-line rule before ending the session. Write it concretely ("Always use X for Y"), never abstractly ("be careful with Y"). If an existing line already covers the correction, tighten it instead of adding a new one. Remove lines when the underlying issue goes away (model upgrades, refactors, process changes).
...
1. specs 位于./docs/superpowers/specs目录下
